...
export interface MobilityBlockSVGProps {
	unit: Unit;
	x?: string;
	y?: string;
}

export const MobilityBlockSVG: React.FC<MobilityBlockSVGProps> = ({
	unit,
	x,
	y,
}: MobilityBlockSVGProps) => {
	return (
		<g transform={`translate(${x || 0} ${y || 0})`}>
			...
			/>
			<rect x="0.125" y="2.75" width="99.75" height="4" rx="0.875" stroke={unit.accentColor} strokeWidth="0.25" fill="rgb(255,255,255)"/>
			...
			{Object.keys(MobilityAttributes).map((key, i) => (
				...
				>
					{`${MobilityAttributes[ key ]}`.toUpperCase()}
				</text>
			))}
